...
def test_merchant_enrichment_clean_record():
    ...
    merchants = [{"merchant_id": "M001", "merchant_name": "Merchant A", "category": "Retail", "city": "New York"}]
    silver = transform_bronze_to_silver(transactions, merchants)
    assert silver[0]["merchant_name"] == "Merchant A"
    assert silver[0]["category"] == "Retail"
    assert silver[0]["city"] == "New York"

def test_unmatched_merchant_gets_flag():
    """Ensure unmatched merchants get quality_flag set to 'UNMATCHED'."""
...
def test_unique_customer_count_per_date():
    """Ensure unique customer count per date is correctly calculated."""
    silver_rows = [
